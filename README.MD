...
Roundtrip testcases were implemented for both, the plain and the encrypted mode.

```
go test ./test -run TestPlaintextRoundtrip

go test ./test -run TestEncryptedRoundtrip
```